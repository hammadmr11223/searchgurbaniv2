...
function BnlVersePrint() {
   // const { type, page_no} = useParams();
      ...
      const page_no = params?.line_no;

      console.log("page_no", page_no,params );
...
const getVerse = async (pageNo) => {
    ...
    await ApiHelper.get(API.getBnlVerse + "?page_no=" + params?.line_no + "&scripture=bnl&type=" + type + "&base_url=bhai-nand-lal")
        .then((resData) => {
            setLoader(false);
            console.log('Ang', resData);
