...
const SGPSGView = (props) => {
 ...
 const router = useRouter();
  const { page_id, volumeno } = router.query;
  const pageno = page_id
    ...
    if (!isNos) {
        return null; // or return a spinner/loader if needed
    }
    return (
        <div>
            <ResourcePage 
                title='Sri Nanak Prakash' 
                pageno={pageno || '1'} 
                volumeNo={volumeno || '1'} 
                apiName='sri-nanak-prakash' 
                nameApi="sri-nanak-prakash/page" 
            />
        </div>
    )
}
