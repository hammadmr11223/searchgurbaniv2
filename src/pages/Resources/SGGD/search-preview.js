...
function SGGDSeachView() {
    ...
    const getSearchResult = async (word, page) => {
        setLoader(true)
        await Axios.get(API.getSGPSGSearchPreview + '?page=' + page + '&keyword=' + word )
            .then((resData) => {
                setLoader(false)
    ...
    return (
        <div>
            ...
            <Head>
                          <title>{`Sri Guru Granth Sahib Darpan Search Preview -: ਸ੍ਰੀ ਗੁਰੂ ਗਰੰਥ ਸਾਹਿਬ ਦਰਪਣ  -: searchgurbani.com `} </title>
                          ...
                          <meta property="og:title" content={`Sri Guru Granth Sahib Darpan Search Preview -: ਸ੍ਰੀ ਗੁਰੂ ਗਰੰਥ ਸਾਹਿਬ ਦਰਪਣ  -: searchgurbani.com `} />
                ...
                <div className='container'>
                    <div className='row'>
                        <div className='in-act-wrapper'>
                            <h2 className='text-dark mb-4'>Sri Guru Granth Darpan</h2>                            
                        </div>
                       <div className='in-act-wrapper'>                       
                            <h6 className='text-dark'>Showing from {pageData.showing_from} to {pageData.showing_to} of occurrences of {word} in Sri Guru Granth Darpan</h6>
                            ...
                            {searchData.length >1 ?
                               ...
                               {/*  <button className='ang-btn' onClick={() => navigate('/sri-gur-pratap-suraj-granth')}>Search Page</button> */}
                                <button className='ang-btn' onClick={() => router.push('/Resources/SGGD')}>Search Page</button>
                            </div>
                        </div>
