...
function SGGKView() {
    ...
    const getSearchResult = async (word, page) => {
        setLoader(true)
        await Axios.get(API.getSGGKresult + '?keyword=' + word + '&alpha=alpha&page=' + page)
            .then((resData) => {
                setLoader(false)
                ...
                <div className='container'>
                    <div className='row'>
                        <div className='in-act-wrapper'>
                            <h2 className='text-dark mb-4'>Sri Guru Granth Kosh</h2>
                        </div>
                        <ul className='letters' >
                            {alphas.map((alpha) => (
                                <li onClick={() => handleAlphaClick(alpha)}><s><b>{alpha}</b></s></li>
                            ))}

                        </ul>
                        <h6 className='text-dark text-center' >Browse by letter</h6>
                        <p className='inner-heading  text-dark' ><h6 className='text-dark'>Here are the results for the letter {word} from Sri Guru Granth Kosh</h6></p>
                        <div className='in-act-wrapper'>
                            ...
                            <div className='actions-mains'>
                               ...
                               { pageData.showing_to !== pageData.total_results ? <button className='ang-btn' onClick={() => handleNextClick(pageNo)}>Next</button> : null }
                                ...
                                <button className='ang-btn' onClick={() => router.push('/Resources/SGGK/SGGKSearch')}>Search Page</button>
                            </div>
                        </div>
