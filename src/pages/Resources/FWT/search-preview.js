...
function FWTSearchView() {
  ...
  const [headingData, setheadingData] = useState();
    ...
    const getSearchResult = async (word, page) => {
        setLoader(true)
        await Axios.get(API.getSNPSearchPreview + '?page=' + page + '&keyword=' + word )
            .then((resData) => {
                setLoader(false)
                ...
                <div className='container'>
                    <div className='row'>
                        <div className='in-act-wrapper'>
                            <h2 className='text-dark mb-4'>Faridkot Wala Teeka</h2>                            
                        </div>
                        <div className='in-act-wrapper'>                       
                            <h6 className='text-dark'><h6 className='text-dark'>Showing from {pageData.showing_from} to {pageData.showing_to} of occurrences of {word} in Faridkot Wala Teeka</h6></h6>
                            ...
                            {searchData.length >1 ?
                               ...
                               { pageData.showing_to !== pageData.total_results ? <button className='ang-btn' onClick={() => handleNextClick(pageNo)}>Next</button> : null }
                                ...
                                <button className='ang-btn' onClick={() => router.push('/Resources/FWT')}>Search Page</button>
                            </div>
                        </div>
