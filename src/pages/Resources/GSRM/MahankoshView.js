...
const transliterateToGurumukhi = (input) =>
  input.split('').map((char) => charMap[char] || char).join('');

export default function MahanKoshView() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const queryWord = searchParams.get('word') || '';

  const [loader, setLoader] = useState(false);
  const [alphas] = useState(['ੳ','ਅ','ੲ','ਸ','ਹ','ਕ','ਖ','ਗ','ਘ','ਙ','ਚ','ਛ','ਜ','ਝ','ਞ','ਟ','ਠ','ਡ','ਢ','ਣ','ਤ','ਥ','ਦ','ਧ','ਨ','ਪ','ਫ','ਬ','ਭ','ਮ','ਯ','ਰ','ਲ','ਵ','ੜ']);
  ...
  useEffect(() => {
    if (word) getSearchResult(word, pageNo);
  }, [word, pageNo]);
    ...
    <>
      ...
      <section className="browse_by_letter p-5">
        <div className="container">
          ...
          <ul className="letters">
            {alphas.map((alpha) => (
              <li key={alpha} onClick={() => handleAlphaClick(alpha)}>
                <Link href={`#`}><b>{alpha}</b></Link>
              </li>
            ))}
          </ul>
          <h6 className="text-dark text-center">Browse by letter</h6>
          <h6 className="text-dark text-center">
            Here are the results for the letter {word}
          </h6>
          <div className="in-act-wrapper">
            <h6 className="text-dark">
              Showing words to {pageData.showing_to} of {pageData.total_results}
            </h6>
            <div className="actions-mains">
              {searchData.length > 1 && (
                <>
                  {pageNo > 0 && <button className="ang-btn" onClick={() => setPageNo(pageNo - 1)}>Previous</button>}
                  {pageData.showing_to !== pageData.total_results && <button className="ang-btn" onClick={() => setPageNo(pageNo + 1)}>Next</button>}
                </>
              )}
              <button className="ang-btn" onClick={() => router.push('/Resources/GSRM/Mahankosh')}>Search Page</button>
            </div>
          </div>
          <div className="ang-display">
            <div className="ang-wrapper">
              {searchData.map((item, idx) => (
                <div className="ang-itm" key={idx}>
                  <Link href={`/GGS/search-results?word=${encodeURIComponent(item.word)}`}>
                   
                      <span className="sg-name" style={{color: '#dd761c'}}><b>{item.word}</b></span>
                   
                  </Link>
                  {' '}– <span>{item.roman}</span> – <span><b>{item.hindi}</b></span>
                  <div className="lang-5">{item.description}</div>
                  <div className="lang-6" style={{ color: '#cd4e06' }}>{item.roman_desc}</div>
                </div>
              ))}
