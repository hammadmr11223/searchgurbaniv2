...
function Tukhari() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=raags/raag-tukhari')
            .then((resData) => {
    ...
    const getAngByAngAudio = async () => {
        setLoader(true)
        await ApiHelper.get(API.getAngByAngAudio + "?path=raags/raag_tukhari")
            .then((resData) => {
                setLoader(false);
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Raag Tukhari -: searchgurbani.com`}
                description={`Learn about Gurbani Raag Tukhari- searchgurbani.com`}
