...
function Maru() {
  ...
  const getData = async () => {
    await Axios.get(
      "https://backend.searchgurbani.com/api/meta?url=raags/raag-maru"
    )
      .then((resData) => {
  ...
  const getAngByAngAudio = async () => {
    setLoader(true);
    await ApiHelper.get(API.getAngByAngAudio + "?path=raags/raag_maru")
      .then((resData) => {
        setLoader(false);
  ...
  return (
    <div>
      <HelmetWrapper
        title={`Raag Maru -: searchgurbani.com`}
        description={`Learn about Gurbani Raag Maru- searchgurbani.com`}
