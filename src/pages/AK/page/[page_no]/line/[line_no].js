...
const PageByPage = (props) => {
    ...
    const handleShareModal = (platform,page,line,punjabi,translit) => {
        ...
        switch (platform) {
            case 'facebook':
                shareLink = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl + `/shared/AK/page/${page}/line/${line}`)}&t=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'twitter':
                shareLink = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl + `/shared/AK/page/${page}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'youtube':
                ...
                break;
            case 'telegram':
                shareLink = `https://telegram.me/share/url?url=${encodeURIComponent(shareUrl + `/shared/AK/page/${page}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'whatsapp':
                shareLink = `https://api.whatsapp.com/send?text=${encodeURIComponent(`${punjabi} ${translit}` + " " + shareUrl + `/shared/AK/page/${page}/line/${line}`)}`;
                break;
            case 'mail':
                shareLink = `mailto:?subject=${encodeURIComponent(`${punjabi} ${translit}`)}&body=${encodeURIComponent(shareUrl + `/shared/AK/page/${page}/line/${line}`)}`;
                break;
            default:
                break;
        }
                        ...
                        <div className='col-lg-12 p-0'>
                            <div className='in-act-wrapper'>
                                ...
                                <div className='actions-mains'>
                                    ...
                                    <button className='action-btn-main' onClick={() => window.open(`/AK/page/${angNo}/print-view`, '_blank', 'height=700,width=700')}><i class="bi bi-printer"></i></button>
                                            ...
                                            <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'} ${parseInt(line_no) === parseInt(item.pagelineno) && 'ang-itm-shabad'}`}>
                                                ...
                                                {isAttrib ?
                                                    <>
                                                        ...
                                                        {/* {isSocialShare ? null : <div style={{ fontSize: `${attribSize}px`, color: attribColor, cursor: 'pointer' }} onClick={(e) => { e.preventDefault(); navigate(`/AK/shabad/${item.shabdID}/${shabadNameWithDashes}/line/${item.shabdlineID}`) }}> Shabad: {item.shabad_name} </div>} */}
                                                        {isSocialShare ? null : <div style={{ fontSize: `${attribSize}px`, color: attribColor, cursor: 'pointer' }} onClick={(e) => { e.preventDefault(); navigate.push(`/AK/shabad/${item.shabdID}/${shabadNameWithDashes}/line/${item.shabdlineID}`) }}> Shabad: {item.shabad_name} </div>}
                                                ...
                                                {isSocialShare ?
                                                    <div className='socia-share' style={{ marginLeft: '600px' }}>
                                                        <ul>
                                                            <li><button className='ang-btn' onClick={(e) => { e.preventDefault(); navigate.push('/AK/shabad/' + `${item.shabdID}` + '/line/' + `${item.shabdlineID}`) }}>Shabad View</button></li>
                                                        </ul>
                                                        <ul>
                                                            <li>
