...
function AkShabadLine() {
    ...
    const shabad_name = params?.shabad_name ;
   ...
   // const title = 'Search Gurbani : Gurbani Website';
    ...
    const handleShareModal = (platform,shabad,line,punjabi,translit) => {
        ...
        switch (platform) {
            case 'facebook':
                shareLink = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl + `/shared/AK/shabad/${shabad}/line/${line}`)}&t=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'twitter':
                shareLink = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl + `/shared/AK/shabad/${shabad}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'youtube':
                ...
                break;
            case 'telegram':
                shareLink = `https://telegram.me/share/url?url=${encodeURIComponent(shareUrl + `/shared/AK/shabad/${shabad}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'whatsapp':
                shareLink = `https://api.whatsapp.com/send?text=${encodeURIComponent(`${punjabi} ${translit}` + " " + shareUrl + `/shared/AK/shabad/${shabad}/line/${line}`)}`;
                break;
            case 'mail':
                shareLink = `mailto:?subject=${encodeURIComponent(`${punjabi} ${translit}`)}&body=${encodeURIComponent(shareUrl + `/shared/AK/shabad/${shabad}/line/${line}`)}`;
                break;
            default:
                break;
        }
                ...
                <div className='container'>
                    <div className='row'>
                        ...
                        <div className='col-lg-6 actions-mains'>
                            ...
                            <button className='action-btn-main' onClick={() => window.open(`/AK/shabad/${shabad_id}/line/${lineno}/print-view`, '_blank', 'height=700,width=700')}><i class="bi bi-printer"></i></button>
                                            ...
                                            <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'} ${parseInt(lineno) === parseInt(item.shabadlineno) && 'ang-itm-shabad'}`}>
                                                ...
                                                {isSocialShare ?
                                                    ...
                                                    <div className='socia-share' style={{ marginLeft: '600px' }}>
                                                        <ul>
                                                            <li><button className='ang-btn' onClick={() => navigate.push(`/AK/page/${item.pageID}/line/${item.pagelineID}`)}>Page View</button></li>
