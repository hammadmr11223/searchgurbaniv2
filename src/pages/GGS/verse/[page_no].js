...
const GGSVerse = (props) => {
    ...
    const getVerse = async (pageNo) => {
        ...
        await ApiHelper.get(API.getGgsVerse + "?page_no=" + pageNo + "&scripture=ggs" + "&base_url=GGS")
            .then((resData) => {
                setLoader(false);
    ...
    const handleShareModal = (platform) => {
        ...
        switch (platform) {
            case 'facebook':
                shareLink = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl + pathname)}&t=${encodeURIComponent(title)}`;
                break;
            case 'twitter':
                shareLink = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl + pathname)}&text=${encodeURIComponent(title)}`;
                break;
            case 'youtube':
                ...
                break;
            case 'telegram':
                shareLink = `https://telegram.me/share/url?url=${encodeURIComponent(shareUrl + pathname)}&text=${encodeURIComponent(title)}`;
                break;
            case 'whatsapp':
                shareLink = `https://api.whatsapp.com/send?text=${encodeURIComponent(title + " " + shareUrl + pathname)}`;
                break;
            case 'mail':
                shareLink = `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(shareUrl + pathname)}`;
                break;
            default:
                break;
        }
                            ...
                            <div className='col-lg-6 p-0'>
                                <div className='in-act-wrapper'>
                                    <div className='actions-mains'>
                                        ...
                                        <button className='action-btn-main' onClick={() => window.open(`/GGS/verse/${page_no}/print-view`, '_blank', 'height=700,width=700')} ><i class="bi bi-printer"></i></button>
                                                ...
                                                {isSocialShare ?
                                                    ...
                                                    <div className='socia-share' style={{ marginLeft: '600px' }}>
                                                        <ul>
                                                            ...
                                                            <li><button className='ang-btn' onClick={(e) => { e.preventDefault(); navigate.push('/GGS/shabad/' + `${item.shabdID}` + '/line/' + `${item.shabdlineID}`) }}>Shabad View</button></li>
